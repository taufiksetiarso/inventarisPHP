<?php 
	...
	if (isset($_GET['deleteMutasiBarang'])){
		$id=$_GET['deleteMutasiBarang'];
		global $con;
		$query="delete from data_mutasi_barang where IdMutasiBarang=$id";
					  ...
					  <tbody>
					    <?php for($i = 0; $i < count($hasil); $i++) : ?>
					    	<tr>
						      ...
						      <td><a href="edit-mutasi-barang.php?editMutasiBarang=<?php echo $hasil[$i]['IdMutasiBarang']; ?>" class="btn btn-success btn-sm">Edit</a> | <a onclick="return confirm('Apakah Yakin Ingin Menghapus?')" class="btn btn-Danger btn-sm" href="data-mutasi-barang.php?deleteMutasiBarang=<?php echo $hasil[$i]['IdMutasiBarang']; ?>">Hapus</a></td>
					    	</tr>
					    <?php endfor; ?>
					  </tbody>
					</table>
